...
Since the datasets of \cite{seitz_et_al06} and \cite{strecha2008} were made available, different approaches addressed dense \mvs, some of which reaching very accurate results, but some issues are still open, \eg, the initialization, the management of untextured regions and the presence of false matches.
...
% They bootstrap form an initial mesh 
...
In our method, named \emph{mesh sweeping}, we refine the mesh by iteratively sweeping its triangle facets around their neighborhood and we look for good stereo matches.
...
\begin{figure}[t]
  \centering
  \includegraphics[width=0.92\columnwidth]{./img/ch-sweep/mesh-sweeping_overview}
...
Let's start from an initial manifold mesh as described in the previous chapter by applying the   \emph{Point Insertion}, \emph{Ray tracing} and \emph{Growing} steps.
...
\begin{figure}[tp]
  \centering
  \includegraphics[width=0.85\textwidth]{././img/ch-sweep/sweep.pdf}
...
\begin{figure}[tp]
  \centering
  \includegraphics[width=0.85\textwidth]{./img/ch-sweep/sweepMulti.pdf}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.85\textwidth]{./img/ch-sweep/sweepingBase}
...
\begin{figure}[t]
  \centering
  \includegraphics[width=0.85\textwidth]{././img/matching.pdf}
...
Then, we iterate for each value $\alpha_j$ and each camera $C_k$ the \emph{Sweeping Iteration} algorithm. 
