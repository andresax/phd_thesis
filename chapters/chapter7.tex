...
For instance, the classical volumetric voxel-based approach requires a huge amount of memory to store accurate and detailed data, but, thanks to the introduction of hashing methods and especially thanks to Delaunay Triangulation based methods, novel algorithms dealing with large-scales scenes have been proposed. 
Very few purely volumetric approaches have been proposed to incrementally build a model of the scene, as the algorithm proposed in this thesis in Chapter \ref{ch:manif}, or in \cite{lovi_et_al_11,hoppe2013incremental,litvinov_lhuillier_13}; however, as these algorithms are  only able to deal with sparse point and provide a rough recosntruction, not comparable to the \mvs results.
...
\section{System Overview}
 Our approach combines three steps  to provide a fully automatic and incremental reconstruction of the environment photo-consistent with the images. In Fig. \ref{fig:architecture} we illustrate a scheme of the whole system. Every  $W$ frames the system outputs the available reconstruction.
...
\section{Incremental photo-consistent refinement}
\label{sec:Incremental_photoconsistent}
Whenever a manifold mesh that represents roughly the observed scene is available, we refine it in order to minimize the photo-metric error between pairwise camera induced by reconstructed surface. 
...
In our case we prefer to mimimize  over the images, as in \cite{pons2007multi}, such that we rely directly on the data. The energy $E_{\textrm{photo}}$ becomes:
\begin{equation}
...
% \begin{equation}
% \label{eq:final}
%   =  - \sum_{i,j} \int_{{\Omega_{i,j}}} \phi_i(x)  f_{ij}(x_i)  \mathbf{N}/z_i^3 \frac{z_i^3}{\mathbf{N}^T \mathbf{d}_i }\mathbf{N} \textrm{d}x_i
% \end{equation}
...
In Figure \ref{fig:openglIncrRef} we illustrate how we implemented the photometric refinement, of a mesh $\mathit{M}^{\text{rec}}$ with respect to two images $I_{1}$ and $I_{2}$ captured by $C_{1}$ and $C_{2}$.
As a first step we send $\mathit{M}^{\text{rec}}$  to  the GPU; we compute the depth of the mesh from the two point of view of camera $C_{1}$ and $C_{2}$ to define the visibility (Depth shaders).
We project $I_{2}$ to the model and we render it to $C_{1}$ (Reprojection shaders) and we compare this projection with the original image $I_{1}$ captured by camera $C_{1}$ to compute the NCC, the block-wise mean and variance  in a $5x5$ px neighborhood (NCC shader). The outputs are needed to compute the $\partial_2 err_{I_i, I_{ij}^{\mathit{S}}}(x_i)$ (Similarity Gradient shader), which in turn is used to compute per-pixel $f_{ij}(x_i)  \mathbf{N}/z_i^3 \frac{z_i^3}{\mathbf{N}^T \mathbf{d}_i }\mathbf{N}$ of Equation \eqref{eq:final} (Gradient Flow shader). 
...
In particular  Vu  \cite{VuPhD011} merges together two mesh refined; the,  he re-triangulates the overlapping areas and he stitches the borders through graph cut-algorithm over the 3D Constrained Delaunay triangulation of the border edges.
...
The dataset is composed by 48 640x480 images; in Table \ref{tab:dinoRes} we listed the comparison with the top ranked algorithm \cite{savinov2016semantic}, which is voxel-based, together with the other mesh-based algorithm closer to our proposal.
