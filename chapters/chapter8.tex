...
We treat the resulting grid as an image and we apply the close morphological operator, to close the small holes and gaps and to filter out isolated points.
We then find connected cells with at least one point populating them and we keep only those regions whose bounding box has a shape compatible with a car. If $h_{BB}$ and $w_{BB}$ are respectively the height and width of the bounding box, we filter out all the regions which do not satisfy:
\begin{equation}
...
\begin{figure}[tp]
 \centering
 \includegraphics[0.8\textwidth]{./img/ch-laser/pointsprojected}
...
\begin{figure}[tp]
 \centering
 \includegraphics[0.8\textwidth]{./img/ch-laser/heightimagePointsProjected_.png}
...
\begin{figure}[tp]
 \centering
 \includegraphics[0.8\textwidth]{./img/ch-laser/pointsprojectedFiltered.png}
