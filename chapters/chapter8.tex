...
As we introduced before, laser scanners and cameras have complementary features; the former are able to provide 3D 360° accurate measurements of the environment, the latter capture the appearance of the environment. Only few authors proposed hybrid approaches to combine laser data with the visual information provided by a camera.
...
\end{figure}
%
\subsection{Point cloud registration and ground plane filtering}
...
Since the ground plane is expected to be mostly static, subsequent laser measurements that lies on the ground plane convey redundant and negligible information about moving objects. Therefore, as a further filtering step, we classify and remove ground plane points. 
...
\begin{figure}[t]
\centering
\includegraphics[width=0.9\columnwidth]{./img/ch-laser/groundpropagation_smal}
...
Let's consider the cell $C_{ij}$ and the set $P_{ij}$ of the points projecting on this cell. 
...
\begin{figure}[t]
\centering
\includegraphics[width=0.9\columnwidth]{./img/ch-laser/scanoccupancy}
\caption{Occupancy at point $P$ computed with respect to the beam $OQ$.}
\label{fig:scanocc}
\end{figure}

Let  $e$~\shorteq~$m(\{empty\})$, $o$~\shorteq~$m(\{occupied\})$ and $u$~\shorteq~$m(\{unknown\})$ be the degrees of belief for the three possible labels for a point such that $e + o + u$~\shorteq~$1$. 
...
\label{sec:images}
Even if the outcome of the laser-based moving object detection algorithm is most of the times satisfying, some false positives may arise due to the noise in the laser measurements and the inaccuracy of the point cloud registration step. 
We propose thus two additional image-based validation tests to filter out false positives: both tests compare image patches around the projection of 3D points classified as moving objects respectively in the (color) images corresponding to the scans in $\mathbb{S}$ and in the depth-maps estimated from the point clouds themselves. If a candidate point passes these tests, then it is a confirmed  moving point.
...
Before computing any similarity between image patches, these are checked for uniformity in their intensities. If their intensity standard deviation is above a certain threshold, then we compare the color patches, otherwise, the test would fail, so we compare directly the depth maps.
...
We opted for NCC measure with respect to the classical Sum of Squared Differences(SSD), since it is not affected by illumination changes, even thought it is computationally more expensive than SSD. Note that, if one of the patches has one of its channels flat, the formula above fails, as we get a division by zero problem for the NCC computation. Our system handles this case with the second test on the depth-maps. 
...
To avoid this undesired behavior, we propose to detect the cars in the point cloud and replace their points with a 3D model of it, \eg, Figure \ref{fig:cardetection2}.

\begin{figure}[tp]
 \centering
\setlength{\tabcolsep}{1px}
    \begin{tabular}{cc}
    \includegraphics[width=0.48\columnwidth]{./img/ch-laser/base00_}&
    \includegraphics[width=0.48\columnwidth]{./img/ch-laser/car00_}\\
    (a)&
    (b)\\
    \includegraphics[width=0.48\columnwidth]{./img/ch-laser/nottext00_}&
    \includegraphics[width=0.48\columnwidth]{./img/ch-laser/text00_}\\
    (c)&
    (d)
 \end{tabular}
...
For each 2D bounding box we project the points along the direction parallel to the longest dimension between $l_{BB}$ and $w_{BB}$ (see Figure \ref{fig:convHull}(a)) in a discrete grid, which is again composed by $0.1m$x$0.1m$ cells.
We compute the convex hull of the projected points (Figure \ref{fig:convHull}(b)) and we treat the result as an image. 
For each column we collect the number of white pixels in a vector $\mathbf{c}$; we compute the discrete derivative of $\mathbf{c}$; then, we compute a three bin histogram of it as in Figure \ref{fig:convHull}(c)).
...
Eventually, we texturize the mesh with a custom method: for each fragment of the mesh, we cumulate the pixel colors from the images where the fragment is visible and does not project into a detected moving point region. 
Let $color_i^f$ be the color of the fragment $f$ into the $i$-th camera, $v_i^f$ represents its visibility, $m_i^f$ is $1$ if $f$ projects int a static region of the image and $0$ if it projects on a moving object region, $\theta$ is the angle between the normal of $f$ and the viewing ray, then the color of $f$ is:
\begin{equation}
 color^f = \frac{\sum_{i=1}^{N} \left\{v_i^f \cdot m_i^f \cdot \cos(\theta)^j \cdot color_i^f\right\}}{\sum_{i=1}^{N}  \left\{v_i^f \cdot m_i^f \cdot \cos(\theta)^j\right\}},
\end{equation}
where $N$ is the total number of cameras. This process weights the colors depending on the perpendicularity of the corresponding viewing ray, we added the exponent $j$, in order to emphasize this weighting process giving much higher weights to the color contribution coming from images facing the mesh perpendicularly (we set experimentally $j=8$).
...
We compare our approach with the state-of-the-art Vallet \etal \cite{vallet2015extracting} which is the approach closer to the proposed.
...
In Table \ref{tab:numPRobj} we show that the proposed algorithm detect or partially detects an higher number of moving objects where  partially detected means that a subset of the moving points remains in the final global cloud.

In Figure~\ref{fig:roc} we report the Receiver Operating Characteristics (ROC) curve obtained with the 0095 sequence: here we compare the ground truth mask against an image-based mask of moving objects obtained a simple dilation of the points classified as moving and projected in the image plane, to have a resutl similar to background subtraction algorithms. 
In the ROC curve, the highest the area subtended by the curve, the better the performance; precision and recall reported in the plot are obtained by varying the $\sigma_r$ and $\sigma_{\theta}$ parameters such that $0.1m<\sigma_r<0.45m$ and $0.0035rad<\sigma_{\theta}<0.0088rad$.
...
The further mage-based validation step requires 0.08 seconds per frame to estimate the depth map and 0.06 seconds to validate the moving points.

The ROC curves also show that both the laser-based pipeline and the image validation procedure improve significantly the precision of the proposed algorithm over the state of the art, in particular the validation has been able to discard a huge number of false positive in moving objects detection.
...
In addition to holes, Ball Pivoting reconstructs a mesh whose normals are not consistent and contains severe self intersections (see Figure \ref{fig:ball}).
...
The method we proposed to texturize the mesh is simple but effective, and it results to be more suitable for urban reconstruction with respect to the more common Mask Photo Blending algorithm proposed in \cite{callieri2008masked}.
Mask Photo Blending have been applied to very dense and accurate model adopted in the Computer Graphics community; in these cases the results of texturing are really accurate and good appealing. In our case, however, we deal with a very different scenario: the model has a lower resolution with respect to the classical Computer Graphics ones, and the images are captured from an inconvenient perspective.
In Figure \ref{fig:mesh1} and Figure \ref{fig:mesh2} we show a comparison of the proposed texturing with respect to the Mask Photo Blending in the KITTI dataset; our approach is able to color more coherently the mesh especially where the model is less accurate.
On the other side, Figure \ref{fig:dwarf} shows that even for a classical Computer Graphics scenarios, the approach we adopted is very close to the output of Mask Photo Blending.
...
\setlength{\tabcolsep}{1px}
    \begin{tabular}{c}
    \includegraphics[width=0.92\columnwidth]{./img/ch-laser/tex01_mesh}\\
    Pose 1, Mask Photo Blending\\
    \includegraphics[width=0.92\columnwidth]{./img/ch-laser/text01_our}\\
    Pose 1, proposed texturing\\
    \includegraphics[width=0.92\columnwidth]{./img/ch-laser/tex02_mesh}\\
    Pose 2, Mask Photo Blending\\
    \includegraphics[width=0.92\columnwidth]{./img/ch-laser/tex02_our}\\
    Pose 2, proposed texturing\\
 \end{tabular}
 \caption{Comparisons of the proposed texturing method with respect to Mask Photo Blending in the KITTI dataset.}
 \label{fig:mesh1}
\end{figure}
...
\setlength{\tabcolsep}{1px}
    \begin{tabular}{c}
    \includegraphics[width=0.92\columnwidth]{./img/ch-laser/tex03_mesh}\\
    Pose 3, Mask Photo Blending\\
    \includegraphics[width=0.92\columnwidth]{./img/ch-laser/tex03_our}\\
    Pose 3, proposed texturing\\
    \includegraphics[width=0.92\columnwidth]{./img/ch-laser/tex04_mesh}\\
    Pose 4, Mask Photo Blending\\
    \includegraphics[width=0.92\columnwidth]{./img/ch-laser/tex04_our}\\
    Pose 4, proposed texturing\\
 \end{tabular}
 \caption{Comparisons of the proposed texturing method with respect to Mask Photo Blending in the KITTI dataset.}
 \label{fig:mesh2}
\end{figure}
...
\setlength{\tabcolsep}{1px}
    \begin{tabular}{cc}
    \includegraphics[height=0.58\columnwidth]{./img/ch-laser/dwarf_mesh}&
    \includegraphics[height=0.58\columnwidth]{./img/ch-laser/dwarf_our}\\
     Mask Photo Blending&
    Pose 1, proposed texturing\\
 \end{tabular}
 \caption{Comparisons of the proposed texturing method with respect to Mask Photo Blending for the dwarf model.}
 \label{fig:dwarf}
\end{figure}
